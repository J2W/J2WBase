...
package j2w.team.common.widget.swipeRefresh;
...
public class SwipeRefreshLayout extends ViewGroup {
	...
	private SwipeProgressBar				mProgressBar;																	// the
																															// thing
																															// that
																															// shows
																															// progress
	...
	private void setTriggerPercentage(float percent) {
		...
		if (((mMode == Mode.PULL_FROM_START) || (mMode == Mode.BOTH)) && mLastDirection != Mode.PULL_FROM_END && !mLoading) {
			mProgressBar.setTriggerPercentage(percent);
		} else if (((mMode == Mode.PULL_FROM_END) || (mMode == Mode.BOTH)) && mLastDirection != Mode.PULL_FROM_START && !mRefreshing) {
			mProgressBarBottom.setTriggerPercentage(percent);
		}
	}
	 ...
	 */
	@Deprecated private void setColorScheme(int colorRes1, int colorRes2, int colorRes3, int colorRes4) {
	 ...
	 */
	public void setTopColor(int colorRes1, int colorRes2, int colorRes3, int colorRes4) {
		setColorSchemeResources(colorRes1, colorRes2, colorRes3, colorRes4);
	}

	public void setBottomColor(int colorRes1, int colorRes2, int colorRes3, int colorRes4) {
	...
	private void setColorSchemeResourcesBottom(int colorRes1, int colorRes2, int colorRes3, int colorRes4) {
		final Resources res = getResources();
		setColorSchemeColorsBottom(res.getColor(colorRes1), res.getColor(colorRes2), res.getColor(colorRes3), res.getColor(colorRes4));
	}
	...
	@Override public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (getChildCount() > 0) {
			getChildAt(0).measure(MeasureSpec.makeMeasureSpec(getMeasuredWidth() - getPaddingLeft() - getPaddingRight(), MeasureSpec.EXACTLY),
	...
	public boolean canChildScrollDown() {
		if (android.os.Build.VERSION.SDK_INT < 14) {
			if (mTarget instanceof AbsListView) {
				...
				if (lastChild != null) {
					return (absListView.getLastVisiblePosition() == (absListView.getCount() - 1)) && lastChild.getBottom() > absListView.getPaddingBottom();
				} else {
					return false;
				}
			} else {
	...
	private void startRefresh() {
		if (!mLoading && !mRefreshing) {
			...
			setRefreshing(true);
			mJ2WPullListFragmentIView.onRefresh();
		}
	}
	...
	private void startLoad() {
		if (!mLoading && !mRefreshing) {
			...
			mReturnToStartPosition.run();
			setLoading(true);
			mJ2WPullListFragmentIView.onLoad();
		}
	}
