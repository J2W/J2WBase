...
public class J2WSyncHandler<T> extends BaseHandler<T> {
	...
	@Override public Object invoke(Object proxy, final Method method, final Object[] args) throws Throwable {
		...
		switch (backgroundType) {
			case HTTP:
				J2WHelper.getThreadPoolHelper().getHttpExecutorService().execute(new J2WAsyncCall(key) {

					@Override protected void execute() {
						L.tag("J2W-Method");
						L.i("网络线程池-并行执行: " + super.mehtodName);
						try {
							if (j2WStack == null || !j2WStack.value()) { // 拦截
			...
			case SINGLEWORK:
				...
				J2WHelper.getThreadPoolHelper().getSingleWorkExecutorService().execute(new J2WAsyncCall(key) {

					@Override protected void execute() {
						L.tag("J2W-Method");
						L.i("工作线程池-串行执行: " + super.mehtodName);
						try {
							if (j2WStack == null || !j2WStack.value()) { // 拦截
								stack.push(super.mehtodName); // 入栈
								method.invoke(t, args);// 执行
				...
				break;
			case WORK:
				J2WHelper.getThreadPoolHelper().getWorkExecutorService().execute(new J2WAsyncCall(key) {

					@Override protected void execute() {
						L.tag("J2W-Method");
						L.i("工作线程池-并行执行: " + super.mehtodName);
						try {
							if (j2WStack == null || !j2WStack.value()) { // 拦截
								stack.push(super.mehtodName); // 入栈
								method.invoke(t, args);// 执行
